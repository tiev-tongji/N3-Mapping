...
def eval_mesh(file_pred, file_trgt, down_sample_res=0.02, threshold=0.05, truncation_acc=0.50, truncation_com=0.50, gt_bbx_mask_on= True, 
              mesh_sample_point=10000000, generate_error_map=False):
    ...
    all_dist_r = np.array(dist_r) # for error map
    dist_r = all_dist_r[all_dist_r < truncation_com]

    error_map = o3d.geometry.PointCloud()
    if generate_error_map:
       error_map = generate_save_error_map(verts_trgt, all_dist_r)
    ...
    metrics = {'MAE_accuracy (cm)': dist_p_mean*100,
...
def generate_mesh_error_map(file_pred, file_trgt, tr=0.50):
    ...
    verts_trgt = np.asarray(pcd_trgt.points)
    _, acc_dist = nn_correspondance(verts_trgt, mesh_verts_pred, tr, False) # find nn in ground truth samples for each predict sample -> precision related
