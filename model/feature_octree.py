...
class FeatureOctree(nn.Module):
    ...
    def __init__(self, config: SHINEConfig):
        ...
        # local leaf node morton list for local mapping
        self.nodes_lookup_tables.append({})
    ...
    def get_octree_nodes(self, level): # top-down
        ...
        node_size = 2**(1-level) # 2/2^{level}, node size in the -1 to 1 kaolin space,
        # nodes_spc_np * node_size is in[0,2], afer minus 1, is in [-1,1]
        # +0.5*node_size means return the center coor of node.
        nodes_coord_scaled = (nodes_spc_np * node_size) - 1. + 0.5 * node_size  
        return nodes_coord_scaled
    
    def get_octree_nodes_spc(self, level):
        octree_data = {}
        nodes_morton = list(self.nodes_lookup_tables[level].keys())
        nodes_morton = torch.tensor(nodes_morton).to(self.device, torch.int64)
        nodes_spc = kal.ops.spc.morton_to_points(nodes_morton)
    ...
    # get the hierachical-sumed interpolated feature at spatial points x
    def query_split_feature_with_indices(self, coord, hierarchical_indices):

        coarse_features = torch.zeros(coord.shape[0], self.feature_dim, device=self.device)
        fine_features = torch.zeros(coord.shape[0], self.feature_dim, device=self.device)
        for i in range(self.featured_level_num): # for each level
            current_level = self.max_level - i
            feature_level = self.featured_level_num-i-1
            # Interpolating
            # get the interpolation coefficients for the 8 neighboring corners, corresponding to the order of the hierarchical_indices
            coeffs = self.interpolat(coord,current_level,self.polynomial_interpolation) 
            if i == 0: # leaf node features
                fine_features = (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).sum(1) 
            else:
                coarse_features += (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).sum(1) 
            # corner index -1 means the queried voxel is not in the leaf node. If so, we will get the trashbin row of the feature grid, 
            # and get the value 0, the feature for this level will then be 0
    ...
    def query_split_feature(self, coord, faster = False):
        self.set_zero() # set the trashbin feature vector back to 0 after the feature update
        if faster:
            indices = self.get_indices_fast(coord)
        else:
            indices = self.get_indices(coord)
        fine_features, coarse_features = self.query_split_feature_with_indices(coord, indices)
        return fine_features, coarse_features
    ...
    def get_indices_fast(self, coord):
        ...
        for i in range(self.featured_level_num): # bottom-up
            ...
            dups_in_mortons = dict(list_duplicates(points_morton)) # list the x with the same morton code (samples inside the same voxel)
    ...
    def print_detail(self):
        ...
        # with open('./log/memory-07.txt', 'a') as f:
        #     f.write(f"{total_map_memory}\n")
        # return