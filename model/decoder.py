...
class Decoder(nn.Module):
    def __init__(self, config: SHINEConfig, is_geo_encoder = True): 
        ...
        # predict residual sdf.
        if config.predict_residual_sdf:
            layers_res = []
            for i in range(mlp_level):
                if i == 0:
                    layers_res.append(nn.Linear(config.feature_dim, mlp_hidden_dim, mlp_bias_on))
                else:
                    layers_res.append(nn.Linear(mlp_hidden_dim, mlp_hidden_dim, mlp_bias_on))
            self.layers_res = nn.ModuleList(layers_res)
            self.lout_res = nn.Linear(mlp_hidden_dim, 1, mlp_bias_on)
    ...
    def sum_sdf(self, fine_features, coarse_features):
        for k, l in enumerate(self.layers):
            if k == 0:
                h = F.relu(l(coarse_features))
            else:
                h = F.relu(l(h))
        coarse_sdf = self.lout(h).squeeze(1)

        for k, l in enumerate(self.layers_res):
            if k == 0:
                x = F.relu(l(fine_features))
            else:
                x = F.relu(l(x))
        res_sdf = self.lout_res(x).squeeze(1)
        
        sdf_out = coarse_sdf + res_sdf
