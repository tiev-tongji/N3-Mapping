...
class Mesher():
            ...
            if iter_n > 1:
                for n in tqdm(range(iter_n)):
                    ...
                    if self.config.predict_residual_sdf:
                        batch_feature, batch_coarse_feature = self.octree.query_split_feature(batch_coord, True) # query features
                        batch_sdf = self.geo_decoder.sum_sdf(batch_feature,batch_coarse_feature)
                    else:
    ...
    def filter_free_space_vertices(self, mesh, verts, map_down_pc):
        
        print("******** begin to clean mesh ********")
        map_down_pc = map_down_pc.voxel_down_sample(0.2 * self.config.mc_res_m)
        print("******** build kd-tree ********")
        points_kd_tree = cKDTree(np.asarray(map_down_pc.points))
        print("******** query neighbors ********")
        radius = min(0.2, self.config.mc_res_m) # 0.15
        verts_find_neighbors_num = points_kd_tree.query_ball_point(verts, 0.80*radius, workers=12, return_length=True)
    ...
    def recon_bbx_mesh(self, bbx, map_down_pc, voxel_size, mesh_path, map_path, \
        save_map = False, estimate_sem = False, estimate_normal = True, \
        filter_isolated_mesh = True, filter_free_space_vertices = False):
        ...
        if filter_free_space_vertices:
            mesh = self.filter_free_space_vertices(mesh, verts, map_down_pc)

        if estimate_normal:
            mesh.compute_vertex_normals()
            #self.visualize_normals_with_rgb(mesh)
    ...
    def recon_octree_mesh(self, query_level, map_down_pc, mc_res_m, mesh_path, map_path, \
                          save_map = False, estimate_sem = False, estimate_normal = True, \
                          filter_isolated_mesh = True, filter_free_space_vertices = False): 
        ...
        # if query_level=tree_level, voxel_count_per_side_node = feature_voxel_size/mc_res_m
