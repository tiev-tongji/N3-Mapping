...
def normal_diff_loss(pred, label, mask):
    # diff1 = (pred - label)[mask].abs().norm(2, dim=1)
    # diff2 = (pred + label)[mask].abs().norm(2, dim=1)
    # diff = torch.min(diff1,diff2)
    diff = (pred - label)[mask].abs().norm(2, dim=1)
    return diff.mean()
